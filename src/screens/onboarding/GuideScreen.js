...
import { markGuideAsSeen } from '../../services/guideService';
...
const GuideScreen = ({ visible, onClose }) => {
  ...
  // 가이드 데이터 - 총 3개 화면으로 분리
  const guides = [
    {
      id: '1',
      type: 'double', // 두 개 기능이 있는 첫 화면
      title: '신한 챗봇 이용 가이드',
      features: [
        {
          title: '대화하기 기능',
  ...
  const finishGuide = async () => {
    try {
      ...
      console.error('가이드 완료 저장 오류:', error);
      onClose();
    }
  };

  // 가이드 항목 렌더링 - 유형에 따라 다른 레이아웃 적용
  const renderGuideItem = ({ item, index }) => {
    const isLastPage = index === guides.length - 1;
    
    // 두 개의 기능이 있는 첫 번째 페이지
    if (item.type === 'double') {
      return (
                ...
                <View style={styles.featureImageContainer}>
                  {item.features[0].image ? (
                    ...
                    <View style={styles.placeholderContainer}>
                      <Text style={styles.placeholderText}>{item.features[0].title} 이미지</Text>
                    </View>
                  )}
                </View>
              </View>
              ...
              <View style={styles.featureSection}>
                ...
                <View style={styles.featureImageContainer}>
                  {item.features[1].image ? (
                    ...
                    <View style={styles.placeholderContainer}>
                      <Text style={styles.placeholderText}>{item.features[1].title} 이미지</Text>
                    </View>
                  )}
                </View>
              </View>
        ...
        <View style={styles.contentContainer}>
          ...
          <View style={styles.guideContentArea}>
            ...
            <View style={styles.singleFeatureSection}>
              ...
              {item.feature.subDescription && (
                <Text style={styles.subDescription}>{item.feature.subDescription}</Text>
              )}
              ...
              <View style={styles.singleFeatureImageContainer}>
                {item.feature.image ? (
                  ...
                  <View style={styles.placeholderContainer}>
                    <Text style={styles.placeholderText}>{item.feature.title} 이미지</Text>
                  </View>
                )}
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  singleFeatureImageContainer: {
    width: '85%',
    height: 180, // 단일 기능 화면에서는 이미지를 더 크게 표시
    alignItems: 'center',
    alignSelf: 'center',
    marginVertical: 20,
  },
